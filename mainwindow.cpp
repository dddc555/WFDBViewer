...
void UI_Mainwindow::add_plif_ecg_filter()
{
    ...
    int sf = this->signalcomp[0]->edfhdr->edfparam[this->signalcomp[0]->edfsignal[0]].sf_int;
    ...
    if((sf % 50) && (sf % 60))  /* don't list signals that can not be filtered */
    {
        ...
        QMessageBox messagewindow(QMessageBox::Critical, tr("Error"), tr("The frequency must an integer ratio to 50 or 60 Hz"));
        messagewindow.exec();
        return;
    }
